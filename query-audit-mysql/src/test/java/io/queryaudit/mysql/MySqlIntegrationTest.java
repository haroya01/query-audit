...
@Tag("integration")
@Testcontainers
@DisplayName("MySQL IndexMetadataProvider integration test")
class MySqlIntegrationTest {
  ...
  static void loadMetadata() throws SQLException {
    MySqlIndexMetadataProvider provider = new MySqlIndexMetadataProvider();
    try (Connection conn =
  ...
  void ordersTableIndexCount() {
    // PRIMARY(id), idx_user_id(user_id), idx_status(status)
  ...
  void primaryKeyDetected() {
    List<IndexInfo> indexes = metadata.getIndexesForTable("users");
    List<IndexInfo> pk =
        indexes.stream().filter(i -> "PRIMARY".equals(i.indexName())).collect(Collectors.toList());

    assertThat(pk).hasSize(1);
    assertThat(pk.get(0).columnName()).isEqualTo("id");
    assertThat(pk.get(0).seqInIndex()).isEqualTo(1);
  ...
  void compositeIndexColumnsInOrder() {
    ...
    IndexInfo first = compositeIdx.stream().filter(i -> i.seqInIndex() == 1).findFirst().get();
    IndexInfo second = compositeIdx.stream().filter(i -> i.seqInIndex() == 2).findFirst().get();

    assertThat(first.columnName()).isEqualTo("status");
    assertThat(second.columnName()).isEqualTo("created_at");
  }
